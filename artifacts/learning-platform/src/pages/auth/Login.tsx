...
import { Eye, EyeOff, Loader2, GraduationCap, Mail, Lock, ArrowRight, SkipForward } from "lucide-react";
    ...
    try {
      ...
      if (authError) {
        if (authError.message.includes("Invalid login credentials")) {
          ...
          setError(t("Please verify your email address first.", "يرجى التحقق من بريدك الإلكتروني أولاً.", "Fadlan hubi emailkaaga marka hore."));
        } else {
      ...
      setError(err.message || t("Login failed. Please try again.", "فشل تسجيل الدخول.", "Galitaanka wuu fashilmay."));
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        <div className="p-8 sm:p-10 rounded-3xl bg-card border border-white/10 shadow-[0_0_80px_rgba(0,0,0,0.5)] backdrop-blur-xl">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.99 }}
            className="w-full py-3 rounded-xl border border-white/10 text-muted-foreground hover:text-white hover:border-white/20 hover:bg-white/5 font-medium text-sm flex items-center justify-center gap-2 transition-all"
          >
