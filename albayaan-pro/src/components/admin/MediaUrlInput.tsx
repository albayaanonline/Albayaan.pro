    ...
    <div className="space-y-2">
      ...
      {mode === "upload" && (
        <div>
          ...
          {uploadStatus === "idle" && (
            <div
              className="flex items-center gap-3 p-4 rounded-xl border-2 border-dashed border-white/10 hover:border-primary/40 cursor-pointer hover:bg-primary/5 transition-all group"
          ...
          {uploadStatus === "done" && (
            <div className="p-4 rounded-xl bg-green-500/5 border border-green-500/20 space-y-2">
              <div className="flex items-center justify-between gap-2">
                ...
                <button type="button" onClick={reset} className="text-xs text-muted-foreground hover:text-white transition-colors shrink-0">
                  Replace
                </button>
              </div>
              {type === "image" && value && (
                <img src={value} alt="" className="w-full max-h-32 object-cover rounded-lg border border-white/10" onError={() => {}} />
              )}
            </div>
          )}
